...
// Define the navigation parameter types here
...
export default function Simulation() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const totalVirus = gridRef.current.flat().reduce((sum, cell) => sum + cell.virus, 0);
      const totalVaccine = gridRef.current.flat().reduce((sum, cell) => sum + cell.vaccine, 0);
      ...
      //console.log('History snapshot:', JSON.parse(JSON.stringify(historyRef.current)));
  ...
  const getCellColor = (cell) => {
    ...
    if (virus > 0 || vaccine > 0) {
      const red = Math.round(255 * (virus / 100));
      const blue = Math.round(255 * (vaccine / 100));
      const green = 0;
      ...
      {!running && (
        ...
        onPress={() => {
          if (historyRef.current && historyRef.current.length > 0) {
            console.log(JSON.parse(JSON.stringify(historyRef.current)));

            router.push({
              pathname: '/Analysis',
              params: { history: JSON.stringify(historyRef.current) }
            });
          } else {
            Alert.alert('No history data to show!');
          }
        }}
      />
